...
def handle_message(message):
    ...
    try:
        system_prompt = SUBJECT_PROMPTS[subject] + "\n\nОтвечай на русском языке. Будь дружелюбным и понятным для школьника. Используй эмодзи. ВАЖНО: не используй LaTeX формулы ($, \frac, \left и т.д.), не используй ### заголовки. Пиши простым текстом без всякой разметки. Формулы пиши словами или обычными символами (например: m/2, 99%, x в степени 2). Не используй звёздочки, решётки и другие спецсимволы для форматирования."
