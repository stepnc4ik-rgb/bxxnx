"""
🎓 Бот-репетитор по всем предметам (Groq)
"""
...
def clean_text(text):
    # $$...$$ -> содержимое (убираем только знаки $$)
    ...
    # \left, \right, \, и прочие служебные без содержимого
    text = re.sub(r'\\(left|right|,|;|!|quad|qquad)', '', text)
    # Оставшиеся LaTeX-команды с аргументом -> только аргумент
    text = re.sub(r'\\[a-zA-Z]+\{([^}]*)\}', r'\1', text)
    # Оставшиеся LaTeX-команды без аргумента -> убираем
...
def handle_message(message):
    ...
    try:
        system_prompt = SUBJECT_PROMPTS[subject] + "\n\nОтвечай на русском языке. Будь дружелюбным и понятным для школьника. Используй эмодзи. ВАЖНО: не используй LaTeX формулы ($, \\frac, \\left и т.д.), не используй ### заголовки. Используй HTML форматирование: <b>жирный</b> для важного, <i>курсив</i> для терминов. Не используй Markdown (**, ##, $). Формулы пиши обычным текстом, например: x = 100, a/b, sqrt(x)."
        ...
        bot.send_message(message.chat.id, clean_text(answer), parse_mode="HTML")
...
# ============================================================
# 📷 ФОТО (Groq поддерживает vision через llama)
# ============================================================
...
def handle_photo(message):
    ...
    try:
        ...
        system_prompt = SUBJECT_PROMPTS[subject] + "\n\nОтвечай на русском языке. Будь дружелюбным. Используй эмодзи. Не используй LaTeX, пиши формулы обычным текстом."
        ...
        data = response.json()
        print(f"Groq vision response: {data}")
        if "choices" not in data:
