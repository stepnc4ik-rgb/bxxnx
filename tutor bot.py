...
def handle_message(message):
    ...
    try:
        system_prompt = SUBJECT_PROMPTS[subject] + "\n\nОтвечай на русском языке. Будь дружелюбным и понятным для школьника. Используй эмодзи. ВАЖНО: не используй LaTeX формулы ($, \frac, \left и т.д.), не используй ### заголовки. Используй HTML форматирование: <b>жирный</b> для важного, <i>курсив</i> для терминов. Не используй Markdown (**, ##, $). Формулы пиши обычным текстом."
